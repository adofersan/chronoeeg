...
class FMMFeatureExtractor(BaseFeatureExtractor):
    ...
    def _estimate_parameters(
        ...
        base = np.apply_along_axis(
            lambda row: self._get_fft_base(dic_an, t, n_obs), 1, base
        )
        ...
        R2comp = self._calculate_r2_index(
            n_channels, self.n_components, t, alphas2, omegas, betas, 
            residuals_fresh, data
        )
    ...
    def _update_channel(
        ...
        for ch in range(n_channels):
            ...
            residuals[ch, :] = (
                (residuals[ch, :] - coefficients[ch, component] * 
                 self._complex_transform(an[component], t))
