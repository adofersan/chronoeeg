...
class QualityConfig:
    ...
    def __post_init__(self):
        ...
        if not 0 <= self.cohesion_threshold <= 1:
            raise ValueError(f"Cohesion threshold must be between 0 and 1, got {self.cohesion_threshold}")
...
class ChronoEEGConfig:
        ...
        return {
            ...
            'features': {
                **self.features.__dict__,
                'freq_bands': dict(self.features.freq_bands),
            },
    ...
    def load_from_yaml(cls, filepath: Path) -> 'ChronoEEGConfig':
        ...
        try:
            ...
            raise ImportError("PyYAML required for loading config. Install with: pip install pyyaml")
